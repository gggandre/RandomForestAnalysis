...
def plot_learning_curves(model, X, y, title=""):
    """
    The function `plot_learning_curves` plots the learning curves of a machine learning model using the
    training and validation scores.
    :param model: The model is the machine learning model that you want to evaluate using learning
    curves. It could be any supervised learning model such as linear regression, decision tree, random
    forest, etc
    :param X: X is the input features or independent variables of the dataset. It should be a
    2-dimensional array-like object, such as a numpy array or a pandas DataFrame, where each row
    represents a sample and each column represents a feature
    :param y: The parameter "y" represents the target variable or the dependent variable in your
    dataset. It is the variable that you are trying to predict or model using the features or
    independent variables in your dataset
    :param title: The `title` parameter is a string that specifies the title of the plot. It is optional
    and can be left empty if you don't want to provide a title for the plot
    """
...
def plot_predictions_vs_actual(model, X_val, y_val, title=""):
    """
    The function `plot_predictions_vs_actual` takes a machine learning model, validation data, and a
    title as input, and plots the predicted values against the actual values.
    :param model: The model parameter is the machine learning model that you want to evaluate. In this
    case, it is a RandomForestRegressor
    :param X_val: The variable `X_val` represents the validation set of input features. It is a matrix
    or dataframe containing the input features for which we want to make predictions
    :param y_val: The variable `y_val` represents the actual values of the target variable in the
    validation dataset. It is used to compare against the predicted values (`y_pred`) generated by the
    model
    :param title: The title of the plot. It is an optional parameter and if not provided, the plot will
    not have a title
    """
...
# The code block is iterating over the `configurations_sample` dictionary, which contains different
# model configurations as keys and corresponding `RandomForestRegressor` models as values.
